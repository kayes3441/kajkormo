...
trait PushNotificationTrait
{
    ...
    protected function referralTransactionStatusUpdateNotification(string $key, string $type, string $lang, int $status, string $fcmToken): void
    {
        $value = $this->pushNotificationMessage($key, $type, $lang);
        if ($value != null) {
            $data = [
                'title' => translate('referral_transaction' . ($status == 1 ? 'release' : 'canceled')),
                'description' => $value,
                'image' => '',
                'type' => 'notification'
            ];
    ...
    protected function pushNotificationMessage(string $key, string $userType, string $lang)
    {
        try {
            $notificationKey = [
                ...
                'referral_transaction_canceled_message'=>'referral_transaction_canceled_message',
                'referral_transaction_release_message'=>'referral_transaction_release_message',
            ];
